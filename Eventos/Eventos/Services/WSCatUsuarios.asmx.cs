...
{
    /// <summary>
    /// Summary description for WSCatUsuarios
    /// </summary>
    ...
    public class WSCatUsuarios : System.Web.Services.WebService
    {

        CatUsuarios dat = new CatUsuarios();
        ...
        [WebMethod(EnableSession = true)]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public string SetCambioPassword(string Parametros)
        {
            string Json_Resultado = string.Empty;
            MensajeServidor ms = new MensajeServidor();
            ...
            try
            {
                JavaScriptSerializer deserializar_json = new JavaScriptSerializer();
                Dictionary<string, object> obj_parametros = deserializar_json.Deserialize<Dictionary<string, object>>(Parametros);
                DataSet ds = dat.SetCambioPassword(obj_parametros);
